...
export const consumerFromJSON = (user: Consumer | undefined, d: any) => {
  const c = new Consumer(user ? user.access_token : d.access_token, user ? user.expires_at : d.expires_at, d.user_info ? `${d.user_info.first_name} ${d.user_info.last_name}` : d.name ?? "", user ? user.image : undefined, d.email, user ? user.requests : undefined)
